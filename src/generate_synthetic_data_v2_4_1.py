...
def collect_real_animal_profile_series(real_animal_files):
    """Extract numeric age/parity from real animal profiles."""
    out = {"age_years": pd.Series(dtype=float), "parity": pd.Series(dtype=float)}
    used = {"age_years": [], "parity": []}

    for path in real_animal_files:
        try:
            df = pd.read_csv(path)
        except Exception:
            continue
        ...
        for feature, aliases in {
            "age_years": ["age", "age_years", "age_year"],
            "parity": ["parity"],
        }.items():
            col = next((cols[a] for a in aliases if a in cols), None)
            if col is None:
                continue
            x = pd.to_numeric(
            ...
            if feature == "age_years":
                ...
                x = x[(x >= 1) & (x <= 8)]
            if len(x):
                out[feature] = pd.concat([out[feature], x], ignore_index=True)
                used[feature].extend([str(path)] * len(x))
    return out, used


def collect_calibration_v24(real_files, public_files):
    """V2.4: real animal profiles anchor static age/parity."""
    cal, sources, file_map = collect_calibration_v23(real_files, public_files)
    real_profiles, real_used = collect_real_animal_profile_series(real_files["animals"])

    for feature in ("age_years", "parity"):
        if len(real_profiles[feature]):
            cal[feature]["real"] = real_profiles[feature]
            cal[feature]["real_n"] = len(real_profiles[feature])
            cal[feature]["source"] = (
                "REAL+PUBLIC" if len(cal[feature]["public"]) else "REAL"
            )
            sources[feature] = cal[feature]["source"]
            file_map[feature]["real"] = real_used[feature]
    return cal, sources, file_map
...
def generate():
    ...
    print("\n=== GoRakshak Synthetic Generator V2.4.1 ===\n")
    ...
    calibration, source_map, file_map = collect_calibration_v24(real_files, public_files)
    ...
    # STATIC animal profile: age and parity are assigned once and never drift.
    base["age_years"] = sample_feature_v23(
        "age_years", N_ANIMALS, calibration["age_years"]
    )
    base["parity"] = np.rint(
        sample_feature_v23("parity", N_ANIMALS, calibration["parity"])
    ).astype(int)
    ...
    for i in event_animals:
        event_day[i] = int(rng.integers(30, N_DAYS - 6))
    ...
    for i, animal in animals.iterrows():
        ...
        for d, date in enumerate(dates, start=1):
            ...
            # Cache empirical bounds once before entering the animal/day loop.
            # This keeps generation fast and avoids repeated pandas quantiles.
            if not hasattr(generate, "_generation_bounds_v23"):
                generate._generation_bounds_v23 = {
                    _feature: v23_bounds(_feature, calibration[_feature])
                    for _feature in FEATURES
                    if _feature != "thi"
                }
            generation_bounds = generate._generation_bounds_v23
            ...
            def bound_feature(value, feature):
                # IMPORTANT: do not recompute pandas quantiles inside the
                # 27,000-row generation loop. V2.3 bounds are cached once.
                b = generation_bounds.get(feature)
                if b:
            ...
            records.append({
                ...
                "day_index": d,
                "age_years": float(base["age_years"][i]),
                "parity": int(base["parity"][i]),
    ...
    out_csv = OUT_ROOT / "gorakshak_synthetic_longitudinal_v4_1.csv"
    out_manifest = OUT_ROOT / "synthetic_animal_manifest_v4_1.csv"
    out_validation = OUT_ROOT / "synthetic_validation_report_v4_1.csv"
    out_report = OUT_ROOT / "synthetic_calibration_report_v4_1.json"
