...
"""
GoRakshak SIH 2026
Synthetic Longitudinal Dataset Generator V2.2
...
def build_animals(real_profiles):
    ...
    for f in ["age_years", "parity"]:
        miss = df[f].isna()
        if miss.any():
            df.loc[miss, f] = sample_feature(f, int(miss.sum()), calibration[f])
        df[f] = np.round(df[f], 2 if f == "age_years" else 0)
        b = robust_bounds(pd.concat([calibration[f]["real"], calibration[f]["public"]], ignore_index=True), f)
        if b:
            df[f] = np.clip(df[f].to_numpy(float), b[0], b[1])
        if f == "parity":
            df[f] = np.rint(df[f]).astype(int)
...
def generate():
    ...
    print("\n=== GoRakshak Synthetic Generator V2.2 ===\n")
    ...
    calibration, source_map, file_map = collect_calibration(real_files, public_files)
    ...
    base = {}
    for f in FEATURES:
        if f in ["age_years", "parity"]:
            continue
        base[f] = sample_feature(f, N_ANIMALS, calibration[f])
    ...
    dim_start = sample_feature("days_in_milk", N_ANIMALS, calibration["days_in_milk"])
    ...
    for i, animal in animals.iterrows():
        ...
        for d, date in enumerate(dates, start=1):
            ...
            # values over 90 days.
            generation_bounds = getattr(generate, "_generation_bounds", None)
            if generation_bounds is None:
                generation_bounds = {}
                for _feature in FEATURES:
                    _combined = pd.concat([
                        calibration[_feature]["real"],
                        calibration[_feature]["public"]
                    ], ignore_index=True)
                    generation_bounds[_feature] = robust_bounds(_combined, _feature) if len(_combined) else None
                generate._generation_bounds = generation_bounds
    ...
    dim_bounds = robust_bounds(
        pd.concat([
            calibration["days_in_milk"]["real"],
            calibration["days_in_milk"]["public"]
        ], ignore_index=True),
        "days_in_milk"
    )
    if dim_bounds:
    ...
    for f in FEATURES:
        if f == "thi":
            # THI is derived. Validate the actual generated THI against the
            # range implied by generated temperature/humidity rather than
            # against an independently sampled THI distribution.
            ...
            b = robust_bounds(pd.concat([
                calibration[f]["real"],
                calibration[f]["public"]
            ], ignore_index=True), f)
            if not b:
                continue
            lower, upper = b
    ...
    out_manifest = OUT_ROOT / "synthetic_animal_manifest_v2.csv"
    out_validation = OUT_ROOT / "synthetic_validation_report_v2.csv"
    out_report = OUT_ROOT / "synthetic_calibration_report_v2.json"
